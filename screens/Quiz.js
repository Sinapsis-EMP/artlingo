...
import { View, Text, SafeAreaView, Dimensions, StatusBar, Image, TouchableOpacity, Modal, Animated, ScrollView, Pressable, Alert } from 'react-native';
...
const Quiz = ({ route }) => {
    ...
    const [UpdateScorez] = useMutation(mutations.UpdateScorez, {
        variables: {
            ...
            puntuacion: ((jugadas) * (Math.pow(presicion, 2.5))) / (Math.pow(velocidad, 2)) * 100,
            partidas: +1
        },
        refetchQueries:
            [
    ...
    async function playSound4() {
        ...
        const { sound } = await Audio.Sound.createAsync(require('../assets/sounds/tic.mp3')
        );
        setSound(sound);
        ...
        return (
            ...
            <View style={{
                ...
                <View style={{
                    ...
                    <Text style={{ color: "#7493BA", fontSize: 20, opacity: 0.6, marginRight: 2 }}>{currentQuestionIndex + 1}</Text>
                    <Text style={{ color: "#7493BA", fontSize: 18, opacity: 0.6 }}>/ {preguntas.length}</Text>
                </View>
        ...
        if (showNextButton) {
            return (<View style={{
                ...
                <TouchableOpacity
                    ...
                    onPress={() => { restartQuiz(), navigation.navigate('Inicio') }}

                    style={{
                        marginTop: 20, marginRight: 5, width: '50%', backgroundColor: COLORS.accent, padding: 20, borderRadius: 5
                    }}>

                    <Text style={{ fontSize: 20, color: COLORS.white, textAlign: 'center' }}>Cerrar</Text>
                </TouchableOpacity>
                <TouchableOpacity

                    onPress={handleNext}
                    style={{
                        alignSelf: "flex-start", marginTop: 20, width: '49%', backgroundColor: "#BE8ABC", padding: 20, borderRadius: 20
                    }}>
    ...
    const renderTime = () => {
        ...
        return (
            <View style={{
                alignItems: "center", alignSelf: "center", width: '100%', padding: 20, borderRadius: 20
            }}>
        ...
        if (showNextButton) {
            ...
            return (
                <View style={{
                    ...
                    <Ionicons name="ios-eye-off" style={{
                        color: "black",
                        fontSize: 40
                    }} />
            ...
            <View style={{
                ...
                >
                    ...
                    <View style={{
                        ...
                        <ConfettiCannon count={200} origin={{ x: -10, y: 0 }} fadeOut={true} fallSpeed={5000} />
                        ...
                        <View style={{
                            ...
                            <Text style={{ fontSize: 30, fontWeight: 'bold' }}>{mensaje}</Text>
                            ...
                            <View style={{
                                ...
                                <Text style={{
                                    fontSize: 22,
                                    color: score > (preguntas.length / 2) ? COLORS.success : COLORS.error
                                }}>Preguntas Contestadas {preguntas.length}</Text>
                            ...
                            <View style={{
                                ...
                                <Text style={{
                                    fontSize: 22,
                                    color: score > (preguntas.length / 2) ? COLORS.success : COLORS.error
                                }}>Preguntas Correctas {score}</Text>
                            ...
                            <View style={{
                                ...
                                <Text style={{
                                    fontSize: 22,
                                    color: score > (preguntas.length / 2) ? COLORS.success : COLORS.error
                                }}>Velocidad Promedio   {(30 - average).toFixed(1)}Segundos</Text>
                            ...
                            <View style={{
                                ...
                                <Text style={{
                                    fontSize: 22,
                                    color: score > (preguntas.length / 2) ? COLORS.success : COLORS.error
                                }}>Puntuación Anterior {Math.floor(puntuacion)}</Text>
                            ...
                            <View style={{
                                ...
                                <Text style={{
                                    fontSize: 22,
                                    color: score > (preguntas.length / 2) ? COLORS.success : COLORS.error
                                }}>Puntuación/Actual{((jugadas) * (Math.pow(presicion, 2.5)) / (Math.pow(velocidad, 2)) * 100).toFixed(2)}</Text>
